...
class PhotoEditor(QMainWindow):
    ...
    def saveImage(self):
        #Save the image. Display error message if image can't be saved.
    ...
    def printImage(self):
        #Print image.
        # Create printer object and print output defined by the platform the program is being run on. QPrinter.NativeFormat is the default
        printer = QPrinter()
        ...
        if (print_dialog.exec_() == QPrintDialog.Accepted):
            ...
            # Set QRect to hold painter's current viewport, which is the image_label
            rect = QRect(painter.viewport())
            # Get the size of image_label and use it to set the size of the viewport
    ...
    def centerMainWindow(self):
        #Use QDesktopWidget class to access information about your screen and use it to center the application window.
